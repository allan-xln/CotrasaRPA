...
function runAllTabs() {
    ...
    var totalTabs = routes.length; // Total de abas a serem executadas
    var completedTabs = 0; // Contador de abas concluídas

    // Função para verificar se todas as abas foram executadas
    function checkAllTabsCompleted() {
        if (completedTabs === totalTabs) {
            stopLoader(); // Para o loader quando todas as abas forem concluídas
            showSuccessMessage(); // Exibe a mensagem de sucesso após rodar todas as páginas
            console.log('Todas as abas executadas com sucesso.');
        }
    }
        ...
        if (index < routes.length) {
            ...
            fetch(routes[index], { method: 'POST' })
                ...
                .then(data => {
                    ...
                    completedTabs++; // Incrementa o contador de abas concluídas
                    checkAllTabsCompleted(); // Verifica se todas as abas foram executadas
                    setTimeout(function () {
